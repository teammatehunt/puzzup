...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--email", help="Override the email that the reminder emails get sent to"
        )
    ...
    def handle(self, *args, **options):
        ...
        testsolve_participations_in_ended = TestsolveParticipation.objects.filter(
            ended__isnull=True
        ).filter(session__in=testsolve_sessions_ended)
        ...
        if options["dry_run"]:
            print(
                "The following user(s) are yet to leave feedback on testsolve session(s):"
            )
        ...
        for t_user_id, t_sessions in user_reminds.items():
            ...
            missing_feedback = [
                ts for ts in t_sessions if ts.session.puzzle.id not in t_spoiled
            ]
            ...
            if options["dry_run"]:
                ...
                if missing_feedback and email_address:
                    ...
                    messaging.send_mail_wrapper(
                        ...
                        {"user": t_user, "outstanding": missing_feedback},
                        [email_address],
                    )
        if options["dry_run"]:
            print(
